...
%
%	Preferences can be found down at the bottom
...
stimulus_location = [window.centerX, window.centerY];
...
for trialIndex = 1:length(prefs.fullFactorialDesign)
  ...
  itemToTest(trialIndex) = randsample(1:nItems,1);
  ...
  gabortex = CreateProceduralGabor(window.onScreen, 100, 100, 0, [0.5 0.5 0.5 0.5]);
  Screen('FillOval', window.onScreen, colorsToDisplay, rects{nItems});
  Screen('DrawTexture', window.onScreen, gabortex, [], [], 90, [], [], [0 0 0], [], kPsychDontDoRotation, [180, .1, 50, 100, 1, 0, 0, 0]);
...
function prefs = getPreferences
  ...
  prefs.stimulusDuration = 1;
  prefs.squareSize = 100; % size of each stimulus object, in pixels
  prefs.radius = 0; %180
  ...
  prefs.colorwheel = prefs.colorwheel.fullcolormatrix + 100;
