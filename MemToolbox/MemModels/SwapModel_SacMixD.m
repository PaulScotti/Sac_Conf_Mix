...
function model = SwapModel_SacMix()
  model.name = 'SwapModel_SacMix';
	model.paramNames = {'muT', 'g', 'R', 'C', 'sdT', 'sdD'};
	model.lowerbound = [-30 0 0 0 0 0]; % Lower bounds for the parameters
	model.upperbound = [30 1 1 1 120 90]; % Upper bounds for the parameters
...
function p = SwapModelPDF(data, muT, g, R, C, sdT, sdD)
  ...
  p = (1-g-R-C).*vonmisespdf(data.errors(:),muT,deg2k(sdT)) + ...
