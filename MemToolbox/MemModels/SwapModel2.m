...
function model = SwapModel()
  model.name = 'Swap model';
	model.paramNames = {'g', 'B', 'sdT', 'sdD'};
	model.lowerbound = [0 0 0 0]; % Lower bounds for the parameters
	model.upperbound = [1 1 90 90]; % Upper bounds for the parameters
	model.movestd = [0.02, 0.02, 0.1, 0.1];
  model.pdf = @SwapModelPDF;
  model.modelPlot = @model_plot;
  model.generator = @SwapModelGenerator;
	model.start = [0.2, 0.1, 10, 10;  % g, B, sd
    0.4, 0.1, 30, 30;  % g, B, sd
...
function p = SwapModelPDF(data, g, B, sdT, sdD)
  ...
  muT = 0; %setting prior mu to be 0
  muD = -90; %biased guessing distribution centered at -90
  ...
  p = (1-g-B).*vonmisespdf(data.errors(:),muT,deg2k(sdT)) + ...
          B.*vonmisespdf(data.errors(:),muD,deg2k(sdD)) + ...
