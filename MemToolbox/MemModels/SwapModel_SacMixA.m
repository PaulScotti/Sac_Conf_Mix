...
function model = SwapModel_SacMix()
  model.name = 'Standard Mixture Model w Bias (model A)';
	model.paramNames = {'muT', 'g', 'sdT'};
	model.lowerbound = [-30 0 0]; % Lower bounds for the parameters
	model.upperbound = [30 1 120]; % Upper bounds for the parameters
	model.movestd = [1 0.02, 0.1];
  model.pdf = @SwapModelPDF;
  model.modelPlot = @model_plot;
  model.generator = @SwapModelGenerator;
	model.start = [0, 0.1, 10;  % muT, g, sdT
    10, 0.2, 20; 
    -10, 0.3, 30]; 
