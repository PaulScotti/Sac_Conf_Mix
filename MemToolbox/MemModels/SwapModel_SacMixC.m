...
function model = SwapModel_SacMix()
  model.name = 'SwapModel_SacMix';
	...
	model.lowerbound = [-30 0 0 0 0]; % Lower bounds for the parameters
	model.upperbound = [30 1 1 120 90]; % Upper bounds for the parameters
	model.movestd = [1, 0.02, 0.02, 0.1, 0.1];
    model.pdf = @SwapModelPDF;
    model.modelPlot = @model_plot;
    model.generator = @SwapModelGenerator;
    model.start =   [0, 0.3, 0.1, 10, 10;  % muT, g, B, sdT, sdD
                    10, 0.2, 0.2, 20, 20; 
                    -10, 0.4, 0.3, 30, 30]; 
...
function p = SwapModelPDF(data, muT, g, B, sdT, sdD)
  ...
  p = (1-g-B).*vonmisespdf(data.errors(:),muT,deg2k(sdT)) + ...
      (B).*vonmisespdf(data.errors(:),90,deg2k(sdD)) + ...
