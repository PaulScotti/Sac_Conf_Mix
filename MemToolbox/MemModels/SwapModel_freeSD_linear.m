...
function model = SwapModel_freeSD()
  model.name = 'Swap model freeSD';
	model.paramNames = {'muT', 'g', 'B', 'sdT', 'sdD'};
	model.lowerbound = [-50 0 0 0 0]; % Lower bounds for the parameters
	model.upperbound = [50 1 1 90 180]; % Upper bounds for the parameters
	model.movestd = [1 0.02, 0.02, 0.1, 0.1];
  model.pdf = @SwapModelPDF;
  model.modelPlot = @model_plot;
  model.generator = @SwapModelGenerator;
	model.start = [0 0.3, 0.3, 10, 20;  % muT, g, B, sdT, sdD
    10 0.2, 0.6, 20, 40; 
    -10 0.6, 0.2, 30, 60]; 
...
function p = SwapModelPDF(data, muT, g, B, sdT, sdD)
  ...
  M = median(abs(data.distractors(:)/180));
  
  p = (1-g-B).*vonmisespdf(data.errors(:),muT,deg2k(sdT)) + ...
      (B).*(abs(data.distractors(:)/180)+M).*vonmisespdf(data.errors(:),data.distractors(1,:)',deg2k(sdD)) + ...
          (g).*unifpdf(data.errors(:), -180, 180);
      
      % .*exp(abs(data.distractors(1,:)'/180))
      %          (B).*vonmisespdf(data.errors(:),data.distractors(:),deg2k(sdD)) + ...

%     numDistractorsPerTrial = 1;
%     pdfOut = vonmisespdf(data.errors(:), data.distractors(:), deg2k(sdD));
