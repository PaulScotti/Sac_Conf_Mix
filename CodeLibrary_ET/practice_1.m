...
maxPracTrials = 24; %12/24 are spatio (50%), just like real exp, should be enough to tell spatio fac
correctFixMinP4 = 20;
...
trialTypeBank = randperm(24);

%% Quest Setup
tGuess = initialOffset; %approx mean
...
gamma=0.5;
grain = stair;
%range is determined later on, since QUEST here requires a single integer
...
Directory.Instruct4 = [Directory.mainDir, filesep, 'InstructableA'];
...
%                 zone = 9;
    ...
    if any(trialType == [1:12])
        control = 0;
        run NS_spatio2
    elseif any(trialType == [13:24])
        control = 1;
        run NS_spatioCtrl2;
    end
    ...
    if time == 2 || time == 3 || time == 4 % after sacc
        ...
        %     REMOVE MEMORY CUE AND REMAIN FIXATED FOR 1000 MS
        Screen('DrawDots', window,firstFix,[10],white, [0,0], 1);
        [timer, stimonset, fliptime, missed, beampos] = Screen('Flip', window);
...
%         end
        ...
        responseDuration_probe = 1.3; % This much time given to make response from stim onset, based off of original paper's raw RTs
        ...
        elseif fixdone == 1
            ...
            look_for_fix = 0;
            FixID = 0;
%             run record_eyes; % instead of keepFix because subject may need to look at keys to respond
            run keepFix; % ???? can subjects respond without looking down at keyboard?
        end
%         if fixdone == 1 || ~ETconnected
